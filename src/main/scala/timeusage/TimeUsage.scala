...
object TimeUsage {
  ...
  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    ...
    */
  def row(line: List[String]): Row = Row.fromSeq(line.head :: line.tail.map(_.trim.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsColumnsPrefixes = List("t01", "t03", "t11", "t1801", "t1803")
    val primaryNeedsColumns: List[Column] = columnNames.collect {
      case columnName if primaryNeedsColumnsPrefixes.exists(columnName.startsWith) => new Column(columnName)
    }

    val workingActivitiesColumnsPrefixes = List("t05", "t1805")
    val workingActivitiesColumns = columnNames.collect {
      case columnName if workingActivitiesColumnsPrefixes.exists(columnName.startsWith) => new Column(columnName)
    }
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = (primaryNeedsColumns.reduce(_ + _) / 60).name("primaryNeeds")  // In hours
    ...
    summed
      ...
      .map {
        case ((working, sex, age), primaryNeeds, work, other) =>
          TimeUsageRow(working = working, sex = sex, age = age, primaryNeeds = primaryNeeds, work = work, other = other)
      }
      .orderBy($"working", $"sex", $"age")
  }
}